<p align="center"><img src="./assets/banner.svg" alt="XXD Panel 081 项目横幅" width="1200"></p>
...
`sample-01`–`sample-04` 保留给未来可核验的原始发布样张。以上八张包括四张 16:9 横版左右样张和四张 3:4 竖版上下样张，均由 Panel 081 依据自己的原始提示词独立生成，不借用其他编号作品。样张使用英文智能配字。
...
```text
提取身份、轮廓、姿态与叙事关系 → 用线把记忆符号化 → 以近等粗单线和开放轮廓画到足够识别 → 用重复符号与线密度聚合复杂区域 → 从原图提取 2–4 种鲜活专色直接编码线条 → 加入颗粒、套印偏差与不均匀着墨 → 围绕一个视觉核心不对称排版 → 让暖白留白与少量手写注记共同叙事
```

## 成品中最容易识别的风格特点

- 外轮廓与内部结构线保持接近的视觉权重，线条描述形状与结构，不塑造真实体积。
- 建筑与器物强调几何骨架，人物、植物、动物与自然形态使用柔软有机曲线。
- 复杂区域通过重复小线性符号和密度层级形成大形，重点密而集中，背景疏而松。
- 彩色线条直接承担信息，不使用“黑色统一描边＋彩色填充”。
- 一个明确视觉核心与不对称组织控制阅读路径，暖白留白承担空气、距离、停顿与情绪。

原始审美约束与拒绝项只存在于[原始提示词](references/original-prompt/zh-CN.md)；Skill 与运行适配器只处理本次交付变量。 [Skill 工作流](SKILL.md) · [英文运行适配器](references/xxd-panel-081-prompt.en.md)
...
| **士兵级** | `xxd-panel-NNN`（当前 001–081） | 每个编号只执行自己独立的原始提示词与审美，把将军派发的单个任务完成为成品。 |
